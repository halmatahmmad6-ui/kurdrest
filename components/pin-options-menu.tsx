...
import { Bookmark, Copy, Download, Eye, EyeOff, Flag, Megaphone, Share2, Trash2 } from "lucide-react";
import { Pin } from "@/lib/types";
import { useAuth } from "@/lib/auth-context";
import { ReportDialog } from "@/components/report-dialog";
import { DeletePinDialog } from "@/components/delete-pin-dialog";
import { PromoteToAdDialog } from "@/components/promote-to-ad-dialog";
...
export function PinOptionsMenu({ pin, trigger, onHide, onDeleted, align = "end" }: PinOptionsMenuProps) {
  ...
  const isOwner = !!user && user.id === pin.author.id && !pin.isAd;
  ...
  return (
    <>
          ...
          >
            ...
            {isOwner && (
              <DropdownMenu.Item
                ...
                className="flex cursor-pointer items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium outline-none hover:bg-surface-hover"
              >
                <Megaphone className="h-4 w-4" /> کردنی بە ڕیکلام
              </DropdownMenu.Item>
            )}
      ...
      <PromoteToAdDialog pinId={pin.id} open={promoteOpen} onOpenChange={setPromoteOpen} />
      <DeletePinDialog
        open={deleteOpen}
