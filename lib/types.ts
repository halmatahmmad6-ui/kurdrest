...
export interface PublicPin {
  ...
  albumId?: string;
  /** present only on the feed's stand-in card for an album — every image in the album, in order */
  albumImages?: { imageUrl: string; mediaType: MediaType; aspectRatio: number }[];
  /** whether the current viewer has liked/saved this pin (false if logged out) */
  likedByMe: boolean;
  savedByMe: boolean;
  createdAt: string;
  /** present only on sponsored feed items */
  isAd?: boolean;
  sponsor?: string;
  linkUrl?: string;
}
