...
export default function SettingsPage() {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    try {
      ...
      if (res.ok) {
        await refresh();
        setMessage("گۆڕانکاریەکان پاشەکەوتکران.");
        router.refresh();
      } else {
  ...
  return (
    <div className="mx-auto max-w-lg py-8">
      <h1 className="mb-6 font-display text-2xl font-extrabold">ڕێکخستنەکانی پرۆفایل</h1>
      ...
      <form onSubmit={handleSave} className="flex flex-col gap-5">
        <div className="flex items-center gap-4">
          ...
          {uploading && <p className="text-sm text-muted-foreground">ئەپلۆدکردن...</p>}
        </div>

        <div>
          <label className="mb-1.5 block text-sm font-semibold">ناو</label>
          <input
            value={name}
        ...
        <div>
          <label className="mb-1.5 block text-sm font-semibold">بایۆگرافیا</label>
          <textarea
            value={bio}
